...
Loads a pre-trained ResNet-18 (ImageNet weights) and adapts it for
CIFAR-10's 32×32 images:
  - Replace conv1 (7×7, stride 2) with 3×3, stride 1, padding 1
  - Remove the max-pool layer
  - Replace the final FC with 10 output classes
...
def get_resnet18_cifar10(pretrained: bool = True) -> nn.Module:
    """
    Return a ResNet-18 adapted for CIFAR-10 (32×32, 10 classes).
    ...
    # Smaller first conv for 32×32 input
    model.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    # Remove aggressive down-sampling
...
def main():
    ...
    print("Building ResNet-18 (ImageNet pre-trained, adapted for CIFAR-10)...")
    model = get_resnet18_cifar10(pretrained=True).to(device)
    ...
    with EnergyTracker(experiment_name="resnet18_baseline") as tracker:
